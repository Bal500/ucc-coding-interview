...
export default function DashboardPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { title: newTitle, start_date: startDate, end_date: endDate, description: newDesc };
  ...
  const handleCalendarEdit = (calEvent: CalendarEvent) => {
      ...
      if (originalEvent) {
        handleEditClick(originalEvent);
      }
  };

  const resetForm = () => {
    setEditId(null); setNewTitle(""); setStartDate(""); setEndDate(""); setNewDesc("");
  };
  ...
  return (
    <div className="min-h-screen bg-black text-gray-100 font-sans p-8 relative">
      ...
      <main className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-[calc(100vh-250px)]">
        ...
        <div className="bg-zinc-900 p-6 rounded-2xl border border-zinc-800 h-fit shadow-xl transition-all">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div><label className="text-xs text-zinc-400">Megnevezés</label><input value={newTitle} onChange={e => setNewTitle(e.target.value)} className="w-full p-2 bg-black border border-zinc-700 rounded text-white mt-1" placeholder="Pl. Meeting" required /></div>
            ...
            <div><label className="text-xs text-zinc-400">Leírás</label><textarea value={newDesc} onChange={e => setNewDesc(e.target.value)} className="w-full p-2 bg-black border border-zinc-700 rounded text-white h-24 mt-1" placeholder="Opcionális..." /></div>
      ...
      {contextMenu && (
        <div 
          className="fixed bg-zinc-800 border border-zinc-600 shadow-2xl rounded-lg overflow-hidden z-[9999] min-w-[150px] animate-in fade-in zoom-in-95 duration-100"
          style={{ top: contextMenu.y, left: contextMenu.x }}
        >
          <button 
            onClick={() => {
              handleCalendarEdit(contextMenu.event);
              setContextMenu(null);
            }}
          ...
          <button 
            ...
            className="w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-red-900/30 flex items-center gap-2 border-t border-zinc-700"
          >
            🗑️ Törlés
          </button>
        </div>
